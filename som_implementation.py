...
if __name__ == "__main__":
    ...
    with graph.as_default():
        ...
        dfx = pd.read_csv(wd + '\\Data\\12k_x.csv', delimiter=',', header=None, index_col=False)
        ...
        tr_x = npx_scaled[:10000]
        te_x = npx_scaled[10000:12000]
    ...
    #quantisation errors
    edlist_x = []
    quantisation_errors_x = []
    for elem in te_x:
        temp_edlist_x = []
        for ele in weights_x:
            ed = eucdist(elem, ele)
            temp_edlist_x.append(ed)
        quantisation_errors_x.append(min(temp_edlist_x))
        edlist_x.append(temp_edlist_x)
    ...
    plt.hist(quantisation_errors_x,bins=100)
    plt.show()

    test_x_timeseries.plot(y=["values"], figsize=(15, 4))
