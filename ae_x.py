...
#convert dataframe to numpy array and scale data for training set
...
encoded = Dense(32, activation='relu')(encoded) #code or bottleneck
...
csv_filepath = os.path.join(model_path,'x_model_1.csv') #training history of model 1 is saved here
...
autoencoder = load_model(os.path.join(model_path, 'model_1.h5'))
...
y_pred = np.array([0 if i<0.0009 else 1 for i in y_pred])
...
x_te3_normal = x_te3_labeled.loc[x_te3_labeled['y_true']==0]
x_te3_anomalous = x_te3_labeled.loc[x_te3_labeled['y_true']==1]
x_te3_warning = x_te3_labeled.loc[x_te3_labeled['y_true']==2]
...
plt.figure(figsize=(10,6))
plt.scatter(x_te3_normal.index, x_te3_normal['x_te3_mse'], label = 'normal')
plt.scatter(x_te3_anomalous.index, x_te3_anomalous['x_te3_mse'], label = 'anomalous')
plt.scatter(x_te3_warning.index, x_te3_warning['x_te3_mse'], label = 'warning')
plt.xlabel('Observations')
plt.ylabel('Reconstruction Error')
plt.title('TE3 - X direction')
plt.legend()
plt.show()
